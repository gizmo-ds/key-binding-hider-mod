...
```json5
{
  ...
  "HiddenCategoryPatterns": [ // Key bindings whose category matches any of these patterns will be hidden.
    "key.categories.multiplayer", // Hide the multiplayer category
    "#jei\\.key\\.category\\..*" // Hide all categories matching this regular expression
  ]
}
```
...
>
> For example, `#key\.jei\..*` hides all JEI key bindings, and `#jei\.key\.category\..*` hides all JEI-related categories.
