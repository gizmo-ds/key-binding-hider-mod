...
public abstract class AbstractComponentBuilder<T> {
    ...
    protected <A extends AbstractConfigListEntry<T>, SELF extends FieldBuilder<T, A, SELF>>
    void fieldBuilderInit(AbstractFieldBuilder<T, A, SELF> builder) {
        ...
        if (language.has(translationKey)) builder.setTooltip(Component.translatable(translationKey));
    }

    public abstract AbstractConfigListEntry<?> build();
}
